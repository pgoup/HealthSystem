...
public class RecipeClientServiceImpl implements RecipeClientService {
  ...
  @Resource private UserCollectRecipesDao userCollectRecipesDao;
  ...
  public RecipeClient getRecipeByNum(String recipeNum, Long userAccount) {
    ...
    if (recipe != null) {
      recipeDao.updateRecipeViewCount(Long.parseLong(recipeNum), recipe.getViewCount() + 1);
    }
  ...
  @Override
  public List<RecipePicAndNameClient> getAllCommonRecipeKind() {
    List<Object[]> recipeKinds = recipeKindDao.getAllCommonRecipeKind();
    System.out.println("种类个数为：" + recipeKinds.size());
    List<RecipePicAndNameClient> recipeMenus = new ArrayList<>();
    for (Object[] objects : recipeKinds) {
      System.out.println(objects[0].toString() + "   " + objects[1]);
      String pic =
          objects[1] == null
              ? "C:\\Users\\PG\\Desktop\\HealthRecipe\\recipe\\recipe10000.png"
              : objects[1].toString();
      recipeMenus.add(
          new RecipePicAndNameClient(objects[0].toString(), ConvertUtils.picConvertToByte(pic)));
    }
    return recipeMenus;
  }
  ...
  public List<RecipeItemClient> getRandomRecipesByUserTags(String tags) {
    ...
    int sum = 0;
    for (String value : values) { // 依次遍历获取标签对应的权重值
      if (!value.isEmpty()) {
    ...
    for (int i = 0; i < vs.size(); i++) {
      int num = weights.get(vs.get(i)) * 20 / sum;
      itemClients.addAll(getRandomCountByKind(vs.get(i), num, weights.get(vs.get(i))));
    }
    Collections.shuffle(itemClients); // 打乱集合元素顺序
    return itemClients;
  }
  ...
  public boolean deleteRecipe(long recipeNum) {
    try {
      System.out.println("进入删除服务端，食谱编号为：" + recipeNum);
      ...
      userCollectRecipesDao.deleteByRecipeNum(recipeNum);
    } catch (Exception e) {
      e.printStackTrace();
      return false;
    }
