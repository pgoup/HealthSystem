...
import com.application.dao.HealthDietNutritionStandardDao;
...
/** @author PG */
@Service
public class HealthComputeImpl implements HealthCompute {
  ...
  @Override
  public String healthDataCompute(
      float height,
    ...
    suggestions.append("&& ").append(computeTagNutrition(nutritions, userAccount));
    return suggestions.toString();
  }

  @Override
  public String computeImportantNutrition(
    ...
    for (UserAddNutrition nutrition : addNutritions) {
      ...
      else if (nutritions.get(nutrition.getNutritionName()) > nutrition.getMaxCount())
        warning.append(nutrition.getNutritionName() + "高于指标值；");
      else warning.append(nutrition.getNutritionName() + "符合指标值；");
    }
    return warning.toString();
  }

  @Override
  public String computeTagNutrition(Map<String, Float> nutritions, Long userAccount) {
    StringBuilder stringBuilder = new StringBuilder();
    UserInfo userInfo = userDao.getUserDetailInfo(userAccount);
    String[] tags = userInfo.getTags().split("&&");
    List<HealthDietNutritionStandard> standards = new ArrayList<>();
    for (String t : tags) {
      HealthDietNutritionStandard s =
          standardDao.getByHealthDietNameAndManKind(t, userInfo.getManKind());
      if (s != null) {
        standards.add(s);
      }
    }
    for (HealthDietNutritionStandard standard : standards) {
      stringBuilder.append("根据标签").append(standard.getHealthDietName()).append(":");
      String heat = standard.getHeat();
      if (heat.contains("-")) {
        String[] value = heat.split("-");
        if (Float.valueOf(value[0]) * userInfo.getWeight() > nutritions.get("热量"))
          stringBuilder.append("热量低于标准值；");
        else if (Float.valueOf(value[1]) * userInfo.getWeight() < nutritions.get("热量"))
          stringBuilder.append("热量高于标准值；");
        else stringBuilder.append("热量符合标准值；");

      } else {
        if (Float.valueOf(heat) * userInfo.getWeight() < nutritions.get("热量"))
          stringBuilder.append("热量高于标准值；");
        else if (Float.valueOf(heat) * userInfo.getWeight() > nutritions.get("热量"))
          stringBuilder.append("热量低于标准值；");
        else stringBuilder.append("热量符合标准值；");
      }
      String fat = standard.getFat();
      if (fat.contains("-")) {
        String[] value = fat.split("-");
        if (Float.valueOf(value[0]) * userInfo.getWeight() > nutritions.get("脂肪"))
          stringBuilder.append("脂肪低于标准值；");
        else if (Float.valueOf(value[1]) * userInfo.getWeight() < nutritions.get("脂肪"))
          stringBuilder.append("脂肪高于标准值；");
        else stringBuilder.append("脂肪符合标准值；");

      } else {
        if (Float.valueOf(fat) * userInfo.getWeight() < nutritions.get("脂肪"))
          stringBuilder.append("脂肪高于标准值；");
        else if (Float.valueOf(fat) * userInfo.getWeight() > nutritions.get("脂肪"))
          stringBuilder.append("脂肪低于标准值；");
        else stringBuilder.append("脂肪符合标准值；");
      }
      String carbohydrate = standard.getCarbohydrate();
      if (carbohydrate.contains("-")) {
        String[] value = carbohydrate.split("-");
        if (Float.valueOf(value[0]) * userInfo.getWeight() > nutritions.get("碳水化合物"))
          stringBuilder.append("碳水化合物低于标准值；");
        else if (Float.valueOf(value[1]) * userInfo.getWeight() < nutritions.get("碳水化合物"))
          stringBuilder.append("碳水化合物高于标准值；");
        else stringBuilder.append("碳水化合物符合标准值；");

      } else {
        if (Float.valueOf(carbohydrate) * userInfo.getWeight() < nutritions.get("碳水化合物"))
          stringBuilder.append("碳水化合物高于标准值；");
        else if (Float.valueOf(carbohydrate) * userInfo.getWeight() > nutritions.get("碳水化合物"))
          stringBuilder.append("碳水化合物低于标准值；");
        else stringBuilder.append("碳水化合物符合标准值；");
      }

      String protein = standard.getProtein();
      if (protein.contains("-")) {
        String[] value = protein.split("-");
        if (Float.valueOf(value[0]) * userInfo.getWeight() > nutritions.get("蛋白质"))
          stringBuilder.append("蛋白质低于标准值；");
        else if (Float.valueOf(value[1]) * userInfo.getWeight() < nutritions.get("蛋白质"))
          stringBuilder.append("蛋白质高于标准值；");
        else stringBuilder.append("蛋白质符合标准值；");

      } else {
        if (Float.valueOf(carbohydrate) * userInfo.getWeight() < nutritions.get("蛋白质"))
          stringBuilder.append("蛋白质高于标准值；");
        else if (Float.valueOf(carbohydrate) * userInfo.getWeight() > nutritions.get("蛋白质"))
          stringBuilder.append("蛋白质低于标准值；");
        else stringBuilder.append("蛋白质符合标准值；");
      }
    }
