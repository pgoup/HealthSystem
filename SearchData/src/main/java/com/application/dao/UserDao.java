...
public interface UserDao extends JpaRepository<UserInfo, Long> {
  ...
  @Query(
      value = "select tags from user_info where user_account=?1 order by id desc  limit 1",
      nativeQuery = true)
  String getTagsByUserAccount(Long userAccount);

  @Query(value = "select * from user_info where user_name = ?1", nativeQuery = true)
  UserInfo getAccountByUserName(Long userName);

  @Query(
      value = "select man_kind from user_info where userAccount=?1 order by id desc limit 1",
      nativeQuery = true)
  Integer getManKindByUserAccount(Long userAccount);
  ...
  @Modifying
  @Query(value = "update user_info set sex=?2 where user_account=?1", nativeQuery = true)
  @Transactional
  void updateUserSex(Long userAccount, String sex);
  ...
  @Modifying
  @Query(value = "update user_info set man_kind=?2 where user_account=?1", nativeQuery = true)
  @Transactional
  void updateUserManKind(Long userAccount, int userWeight);
