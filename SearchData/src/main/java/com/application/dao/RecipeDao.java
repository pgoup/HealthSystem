...
public interface RecipeDao extends JpaRepository<Recipe, Long> {
  ...
  @Query(
      value =
          "select recipe_name,pic_path,kind,view_count,collect_count,recipe.recipe_num from recipe,recipes_and_kinds  where recipe.recipe_num=recipes_and_kinds.recipe_num and recipes_and_kinds.recipe_kind_num=?1 ",
      countQuery =
  ...
  @Query(
      value =
          "select recipe_name,pic_path,kind,view_count,collect_count,recipe.recipe_num from recipe , health_diet_recipe where recipe.recipe_num=health_diet_recipe.recipe_num and health_diet_recipe.health_diet_num=?1",
      countQuery =
  ...
  @Query(
      value =
          "select recipe_name,pic_path,kind,view_count,collect_count,recipe.recipe_num from recipe , function_benefit_recipe where recipe.recipe_id=function_benefit_recipe.recipe_id and function_benefit_recipe.function_benefit_id=?1",
      countQuery =
  ...
  @Query(
      value =
          "select recipe_name,pic_path,kind,view_count,collect_count,recipe.recipe_num from recipe  order by collect_count desc limit 500",
      nativeQuery = true)
  List<Object[]> getRecipesByCollectCount();

  @Query(
      value =
          "select recipe_name,pic_path,kind,view_count,collect_count,recipe.recipe_num from recipe  join recipes_and_kinds where recipe.recipe_num=recipes_and_kinds.recipe_num and recipes_and_kinds.recipe_kind_num=?1",
      nativeQuery = true)
  List<Object[]> getRecipesByKindNum(int kindNum);

  @Query(
      value =
          "select recipe_name,pic_path,kind,view_count,collect_count,recipe.recipe_num from recipe where recipe_name=?1 or key_ingredient=?1 ",
      countQuery = "select count(*) from recipe where reipe_name=?1 or key_ingredient=?1",
      nativeQuery = true)
  ...
  @Query(
      value =
          "select recipe_name,pic_path,kind,view_count,collect_count,recipe.recipe_num from recipe where recipe_num=?1 limit 1",
      nativeQuery = true)
