...
public interface UserRecipesDao extends JpaRepository<UserRecipes, Long> {
    ...
    @Query(
            value =
                    "select recipe_name,pic_path,kind,view_count,collect_count,recipe.recipe_num from recipe , user_recipes where recipe.recipe_num=user_recipes.recipe_num and user_recipes.user_account=?1",
            countQuery =
