...
public interface UserCollectRecipesDao extends JpaRepository<UserCollectRecipes, Long> {
  ...
  @Transactional
  @Query(
      value = "delete from user_collect_recipes where user_account=?1 and recipe_number=?2",
      nativeQuery = true)
  void deleteByAccountAndRecipeNum(Long userAccount, Long recipeNum);

  @Query(
      value =
          "select recipe_name,pic_path,kind,view_count,collect_count,recipe.recipe_num from recipe , user_collect_recipes where recipe.recipe_num = user_collect_recipes.recipe_num and user_collect_recipes.user_account = ?1",
      countQuery =
