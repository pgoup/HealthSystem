...
public class UserController {
  ...
  public boolean saveUserInfo(HttpServletRequest request) {
    ...
    return userService.saveUserInfo(Long.valueOf(account), password);
  }
  ...
  private void addUserTags(HttpServletRequest request) {
    ...
    UserInfo userInfo = userService.getUserInfoByAccount(userAccount);
    if (userInfo.getTags() == null) userInfo.setTags("");
  ...
  private boolean updateUserConcern(HttpServletRequest request) {
    ...
    if (isConcerned) return userService.deleteConcernedAuthor(userAccount, concernedUserAccount);
    else userService.addConcernedAuthor(userAccount, concernedUserAccount);
    return true;
  }

  @RequestMapping(value = "/deleteFanOrConcern", method = RequestMethod.POST)
  @ResponseBody
  private boolean deleteFanOrConcern(HttpServletRequest request) {
    Long userAccount = Long.parseLong(request.getParameter("userAccount"));
    Long conOrFanUserAccount = Long.valueOf(request.getParameter("fanOrConcernAccount"));
    String kind = request.getParameter("kind");
    if (kind.equals("concern"))
      return userService.deleteConcernedAuthor(userAccount, conOrFanUserAccount);
    else return userService.deleteConcernedAuthor(conOrFanUserAccount, userAccount);
  }
  ...
  private RecipeNutritionSuggestion getRecipesNutritionByRecipeNums(HttpServletRequest request) {
    ...
    String recipeValue = request.getParameter("recipes");
    System.out.println(recipeValue);
    List<String> recipes = JSONObject.parseArray(request.getParameter("recipes"), String.class);
    return userService.getRecipesNutritionByRecipeNums(userAccount, recipes);
  }

  @ResponseBody
  @RequestMapping(value = "/saveUserNutrition", method = RequestMethod.POST)
  private boolean saveUserNutrition(HttpServletRequest request) {
    String name = request.getParameter("name");
    String userAccount = request.getParameter("userAccount");
    float min = Float.valueOf(request.getParameter("min"));
    float max = Float.valueOf(request.getParameter("max"));
    return userService.saveUserNutrition(userAccount, name, min, max);
  }

  @ResponseBody
  @RequestMapping(value = "/getUserNutrition", method = RequestMethod.POST)
  private List<UserAddNutrition> getUserNutrition(HttpServletRequest request) {
    String userAccount = request.getParameter("userAccount");
    return userService.getUserAddNutritions(userAccount);
  }

  @ResponseBody
  @RequestMapping(value = "/deleteUserAddNutrition", method = RequestMethod.POST)
  private boolean deleteUserAddNutrition(HttpServletRequest request) {
    System.out.println("进入删除控制单");
    String userAccount = request.getParameter("userAccount");
    String nutritionName = request.getParameter("nutritionName");
    return userService.deleteUserAddNutrition(userAccount, nutritionName);
  }
}
