...
public class HealthDataRecipeContentAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public List<RecipeItemClient> getContentItems() {
        return contentItems;
    }
        ...
        if (holder instanceof ContentViewHolder) {
            ...
            viewHolder.addRecipeImage.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    bundle.putString("contents", JSONObject.toJSONString(contentItems));
                    if (kind == 1) {
                        ...
                        Intent intent = new Intent();
                        intent.setAction("DeleteUserRecipeAction");
                        intent.putExtras(bundle);
                        LocalBroadcastManager.getInstance(context).sendBroadcast(intent);
                    }
                }
    ...
    public int getItemViewType(int position) {
        if (position == contentItems.size() || contentItems.size() == 0) hasMore = false;

        if (kind != 2) {
            if (0 <= position && position < contentItems.size()) {
                return CONTENT_TYPE;
            } else {
                if (contentItems.size() == 0) hasMore = false;
                return MORE_TYPE;
            }
        } else
            return CONTENT_TYPE;
