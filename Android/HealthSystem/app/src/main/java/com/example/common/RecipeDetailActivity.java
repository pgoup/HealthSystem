...
public class RecipeDetailActivity extends AppCompatActivity {
    ...
    private void initView() {
        ...
        collectButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (!UserManager.getInstance().hasUserInfo(RecipeDetailActivity.this)) {//本地没有用户信息，用户尚未登陆，提示用户登录
                    InternetTooltip.tip(RecipeDetailActivity.this, "您还未登录，请先登录");
                    return;
                }
                String param = "userAccount=" + UserManager.getInstance().getUserInfo(RecipeDetailActivity.this).getAccount()
                        + "&collectRecipeNum=" + recipeNum + "&isCollected=" + isCollected;//构建参数，向服务端发送请求信息
                ...
                if (result == null) {
                    ...
                    isCollected = !isCollected;//改变收藏状态
                    changeState(isCollected, "1已收藏", "1收藏", "收藏", (int) concernButton.getTextSize());//更改美食详情页收藏按钮文本
                    Long num = Long.valueOf(recipeCollectCount.getText().toString());
                    refresh(isCollected ? num + 1 : num - 1);//更改美食详情页收藏次数文本
