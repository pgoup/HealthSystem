...
public class MimeMyRecipesActivity extends AppCompatActivity implements SwipeRefreshLayout.OnRefreshListener  {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        intentFilter.addAction("DeleteRecipeAction");
    ...
    private class BroadCastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals("DeleteRecipeAction")) {
                ...
                if (result.equals("OK")) {
                    try {
                        System.out.println("数据量为:" + contents.size());
                        int num = 0;
                        for (int i = 0; i < contents.size(); i++) {
                            if (contents.get(i).getNum().equals(recipeNum)) {
                                num = i;
                                break;
                            }
                        }
                        contents.remove(num);
                        System.out.println("删除后的数据量为：" + contents.size());
                        viewAdapter.setContentItems(contents);
                        viewAdapter.notifyDataSetChanged();
                    } catch (Exception e) {
                        e.printStackTrace();
