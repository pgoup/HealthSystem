...
public class MimeLoginedFragment extends Fragment {
    ...
    public View onCreateView(final LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        if (userInfo.getPic() == null) {
            InternetTooltip.tip(getActivity(), InternetTooltip.DATA_ERROR);
        } else {
        ...
        intentFilter.addAction("ChangeConcernAction");
        intentFilter.addAction("UpdateMimeFragmentAction");
        broadcastManager.registerReceiver(broadCastReceiver, intentFilter);

        intro = view.findViewById(R.id.mime_intro);
        intro.setText(userInfo.getIntro());
        view.findViewById(R.id.mime_intro_text).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                changeIntro();
            }
        ...
        logOut.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                MainActivity.refreshMimeFragment(context);
    ...
    private void uploadPic(Bitmap picBitmap) {
        ...
        if (result == null || result.equals("false")) {
            ...
            InternetTooltip.tip(context, "上传成功！");
    ...
    private void uploadIntro(String introText) {
        ...
        if (result == null || result.equals("false")) {
            ...
            dialog.dismiss();
            InternetTooltip.tip(context, "提交成功！");
        }
    ...
    private class BroadCastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals("UpdateInformation")) {
                String name = intent.getExtras().get("userName").toString();
                System.out.println("修改后的用户名为：" + userName);
                userName.setText(name);
            }
            if (intent.getAction().equals("ChangeConcernAction")) {
                String kind = intent.getExtras().get("kind").toString();
                if (kind.equals("concern")) {
                    concernedCount.setText(String.valueOf(Integer.valueOf(concernedCount.getText().toString()) - 1));
                } else {
                    collectView.setText(String.valueOf(Integer.valueOf(concernedCount.getText().toString()) - 1));
                }
            }
            if (intent.getAction().equals("UpdateMimeFragmentAction")) {
                String param = "userAccount=" + UserManager.getInstance().getUserInfo(getActivity()).getAccount();
                String result = HttpClientUtils.httpPostRequest(HttpClientUtils.serverUrl + "getUserInfoClient", param);
                if (result == null) {
                    InternetTooltip.tip(getActivity(), HttpClientUtils.HTTP_REQUEST_ERROR);
                ...
                if (userInfo.getPic() == null) {
                    ...
                    hierarchy.setRoundingParams(params);
                    userImage.setHierarchy(hierarchy);
                    String para = "userAccount=" + UserManager.getInstance().getUserInfo(getActivity()).getAccount();
                    List<String> values = new ArrayList<>();
                    String result1 = HttpClientUtils.httpPostRequest(HttpClientUtils.serverUrl + "getUserTags", para);
                    if (result1 == null) {
                        InternetTooltip.tip(getActivity(), HttpClientUtils.HTTP_REQUEST_ERROR);
                    } else {
                        System.out.println("result为：" + result1);
                        List<String> re = JSONArray.parseArray(result1, String.class);
                        System.out.println("集合的数量为：" + re.size());
                        if (re.size() > 0) values.addAll(re);
                        values.remove("");
                        values.add("添加");
